...
BLEUart bleuart; // uart over blel
 ...
 while ( bleuart.available() )
  {
    ...
    while (st != NULL) {
      ...
      switch (p)
      {
          ...
          else if (strcmp(st, "save") == 0) {
          bleuart.write("Saving file via\n\n",17);
          bleuart.write("Send u/ >>> USB\n",16);
          bleuart.write("Send b/ >>> BLE\n",16);
          state = false;
          Bluefruit.autoConnLed(true);
          Save_File_Serial(); 
          }               
...
void find_last_file(){
  ...
  check_SD();
  do {
  sprintf(last_filename,"LS13_%d.csv", i); 
  i++;
}
while (SD.exists(last_filename));
  sprintf(last_filename,"LS13_%d.csv", (i-2));

  }

void Save_File_Serial(){
  char save_ty[3];
  char save_print[34];
   bool save = false;
  while(!save){
 ...
 find_last_file();
 sprintf(save_print, "Saving %s via %s ... \n\n", last_filename, save_ty);
 bleuart.write(save_print,34);
 Serial.println(save_print);
 //Serial.println(last_filename);
 //bleuart.write(last_filename,13);
 //bleuart.write("\n",1);c
 delay(2000);
 File dataFile = SD.open(last_filename);
  ...
  if (dataFile) {
    ...
    dataFile.close();
    bleuart.write("File saved.\n",13);
    bleuart.write("Now you can export the file.\n\n",31);

  }  
  // if the file isn't open, pop up an error:
  else {
    char err_sav_msg[48];
    sprintf(err_sav_msg,"Saving file:""%s"" failed.\n",last_filename);
    Serial.write(err_sav_msg);
    //bleuart.write(err_sav_msg,48);
  }
  save = false;
  } 
